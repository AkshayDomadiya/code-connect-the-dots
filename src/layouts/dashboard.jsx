...
export function Dashboard() {
  ...
  const getFilteredRoutes = (routes, includeHidden = false) => {
    return routes.map(section => ({
      ...section,
      pages: section.pages.filter(page =>
        page.roles?.includes(userRole) && (includeHidden || !page.hidden)
      )
    }));
  };

  const filteredRoutesForSidebar = getFilteredRoutes(routes); // Only visible pages
  const filteredRoutesForRouting = getFilteredRoutes(routes, true); // All pages, including hidden
